...
Parameters for both attempts: paid generation, 12.5 credits each; `aspect_ratio=16:9`; `duration=5`; `resolution=720p`; `1280x720`; `24 fps`; `bitrate_mode=high`; `genre=drama`; `generate_audio=false`; `count=1`.
...
- Review: accepted after full-sequence and contact-sheet inspection. The pen geometry and woven surface remain coherent, the magenta-to-blue mark is clean, and the frame contains no text, numbers, logos, people, alcohol or typical AI artefacts. The left half provides strong copy contrast and the action survives a centre-biased mobile crop. Published derivatives are a `1280x720` H.264 MP4 and VP9 WebM without audio. They use a forward/reverse edit for a jump-free 10-second loop; a WebP poster is included for loading and reduced-motion presentation.
