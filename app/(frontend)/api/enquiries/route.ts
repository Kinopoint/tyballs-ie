...
export async function POST(request: NextRequest) {
  ...
  let cms: Awaited<ReturnType<typeof getCms>>;
  ...
  try {
    ...
    await cms.create({
      ...
      data: {
        ...
        contact: {
          ...
          contactName: `${enquiry.firstName} ${enquiry.lastName}`,
          email: enquiry.email,
          phone: enquiry.phone,
        },
        contactName: `${enquiry.firstName} ${enquiry.lastName}`,
        schoolDetails: {
          ...
          county: enquiry.schoolLocation,
          joiningSchools: enquiry.joiningSchools || undefined,
          yearSize: enquiry.yearSize,
        },
        school: enquiry.school,
        county: enquiry.schoolLocation,
        eventDetails: {
          eventType: enquiry.enquiryType,
          preferredDate: enquiry.preferredDate,
          preferredLocation: enquiry.preferredLocation,
          attendanceBand: enquiry.attendanceBand,
          estimatedAttendance: attendanceEstimate[enquiry.attendanceBand],
          message: enquiry.message || undefined,
        },
        preferredDate: enquiry.preferredDate,
        source: {
          referralSource: enquiry.referralSource,
          referralOther: enquiry.referralOther || undefined,
          landingPage: enquiry.landingPage || undefined,
          ...
          utmContent: enquiry.utmContent || undefined,
          utmTerm: enquiry.utmTerm || undefined,
    ...
    await client.query("ROLLBACK");
    if (cmsSaved && !committed) {
      try {
        await cms.delete({ collection: "enquiries", id, context: { skipLegacySync: true }, overrideAccess: true });
      } catch (cleanupError) {
        console.error("CMS enquiry rollback error", { enquiryId: id, cleanupError });
      }
    }
